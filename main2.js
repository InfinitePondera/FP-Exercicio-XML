...
const appsAddedDate = elementsToArray(document.getElementsByTagName('added'))
    .filter(isAddedAfter2000AndUpdatedAfter2020)
//    .map(content);

const appsUpdatedDate = elementsToArray(document.getElementsByTagName('lastupdated'))
    .filter(isAddedAfter2000AndUpdatedAfter2020)

var results;
results.join(addedApps + ' ');
results.join(appsAddedDate + ' ');
results.join(appsUpdatedDate + '\n');
